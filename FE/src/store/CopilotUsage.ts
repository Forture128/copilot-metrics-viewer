...
const actions = {
  ...
  async fetchData(
    { commit }: { commit: Commit },
    {
      ...
      mutation,
      readyMutation
    }: { apiCall: () => Promise<any>; mutation: string; readyMutation: string }
  ) {
  ...
  async fetchAllTeamMetrics({
    ...
    try {
      ...
      const fetchTeams = state.teamList
        .filter(team => team !== 'All Teams')
        .map(team => dispatch('fetchTeamMetrics', team))
...
const getters = {
  ...
  getDepartmentTeams: (state: State) => (department: string) => state.departments[department] || []
}
