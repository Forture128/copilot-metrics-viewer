    ...
    <v-container fluid class="charts-container">
        ...
        <v-col cols="12" md="10">
          <v-card class="chart-card">
            <v-card-title class="text-center">
              {{ breakdownDisplayNamePlural }} Breakdown Bar Chart
            </v-card-title>
            <v-card-text>
              <BarBreakdown :data="breakdownsChartDataTop20AcceptedData" :options="chartOptions" />
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>
      <v-row v-if="breakdownKey != 'editor'" justify="center">
        <v-col cols="12" md="10">
          <v-card class="chart-card">
            <v-card-title class="text-center">
              {{ breakdownDisplayNamePlural }} Breakdown Line Chart AcceptedPrompt Metrics
            </v-card-title>
            <v-card-text>
              <LineBreakdown
                :data="breakdownsChartDataTop20AcceptedPromptData"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const totalAcceptedPrompts = computed(() =>
      Array.from(breakdowns.value.values()).reduce((acc, item) => acc + item.acceptedPrompts, 0)
    )
    const totalAcceptedLinesOfCode = computed(() =>
      Array.from(breakdowns.value.values()).reduce((acc, item) => acc + item.acceptedLinesOfCode, 0)
    )
    const totalSuggestedLinesOfCode = computed(() =>
      Array.from(breakdowns.value.values()).reduce(
        (acc, item) => acc + item.suggestedLinesOfCode,
        0
      )
    )
    ...
    breakdowns.value = new Map(
      [...breakdowns.value.entries()].sort((a, b) => b[1].acceptanceRate - a[1].acceptanceRate)
    )
    ...
    breakdowns.value = new Map(
      [...breakdowns.value.entries()].sort((a, b) => b[1].acceptedPrompts - a[1].acceptedPrompts)
    )
    ...
    const top20BreakdownsAcceptedPrompts = new Map([...breakdowns.value].slice(0, 20))

    breakdownsChartData.value = {
      labels: Array.from(top20BreakdownsAcceptedPrompts.values()).map(breakdown => breakdown.name),
      datasets: [
        {
