      ...
      <v-col cols="12" md="6">
        <v-row class="cycle-metrics">
          <v-col cols="12" class="cycle-metric-time" style="text-align: center; padding: 20px">
            <span class="main-time" style="font-size: 2.5em; font-weight: bold">{{
              formattedCycleTime
            }}</span>
            <span
              class="time-change positive"
              style="font-size: 1.5em; color: green; margin-left: 10px"
              >▼ 70 %</span
            >
          </v-col>
          <v-col cols="12">
            <v-row justify="space-between" class="metric-breakdown">
              <v-col cols="3" class="text-center">
                <div class="metric-time" style="color: #f44336">
                  {{ phaseTimes.coding }}
                </div>
                <div
                  class="rectangle"
                  style="background-color: #f44336; height: 8px; width: 100%"
                />
                <small>Coding</small>
              </v-col>
              <v-col cols="3" class="text-center">
                <div class="metric-time" style="color: #4caf50">
                  {{ phaseTimes.pickup }}
                </div>
                <div
                  class="rectangle"
                  style="background-color: #4caf50; height: 8px; width: 100%"
                />
                <small>Pickup</small>
              </v-col>
              <v-col cols="3" class="text-center">
                <div class="metric-time" style="color: #ffeb3b">
                  {{ phaseTimes.review }}
                </div>
                <div
                  class="rectangle"
                  style="background-color: #ffeb3b; height: 8px; width: 100%"
                />
                <small>Review</small>
              </v-col>
              <v-col cols="3" class="text-center">
                <div class="metric-time" style="color: #2196f3">
                  {{ phaseTimes.merge }}
                </div>
                <div
                  class="rectangle"
                  style="background-color: #2196f3; height: 8px; width: 100%"
                />
                <small>Merge</small>
              </v-col>
            </v-row>
    ...
    const updatePhaseTimes = async () => {
      ...
      for (const pr of props.pullRequests) {
        ...
        if (reviews) {
          ...
          pickupTimes.push(
            calculateTimeDifferenceInDays(reviews.first_review_at, reviews.review_completed_at)
          )
          reviewTimes.push(
            calculateTimeDifferenceInDays(reviews.review_completed_at, reviews.approved_at)
          )
          mergeTimes.push(calculateTimeDifferenceInDays(reviews.approved_at, pr.merged_at))
        }
      }

      const averageCodingTime =
        codingTimes.length > 0 ? codingTimes.reduce((a, b) => a + b, 0) / codingTimes.length : 0
      const averagePickupTime =
        pickupTimes.length > 0 ? pickupTimes.reduce((a, b) => a + b, 0) / pickupTimes.length : 0
