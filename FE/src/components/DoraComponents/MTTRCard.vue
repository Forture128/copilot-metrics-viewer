    ...
    <v-row>
          ...
          <v-col cols="12">
            <v-row justify="space-between" class="metric-breakdown">
              <v-col cols="12" class="text-center">
                <small
                  >Mean Time to Restore: <b>{{ formattedMTTR }}</b></small
                >
              </v-col>
            </v-row>
          </v-col>
      ...
      <v-col cols="12" md="6">
        <div style="width: 100%; position: relative">
          <MTTRLine :data="mttrData" :options="chartOptions" />
        </div>
      </v-col>
    </v-row>
  </v-card>
  ...
  setup(props) {
    ...
    const formattedMTTR = computed(() => {
      ...
      failedDeployments.forEach(failure => {
        ...
        if (recovery) {
          const timeToRestore = calculateTimeDifferenceInHours(
            failure.created_at,
            recovery.created_at
          )
      ...
      const averageMTTR =
        mttrList.length > 0 ? mttrList.reduce((a, b) => a + b, 0) / mttrList.length : 0
