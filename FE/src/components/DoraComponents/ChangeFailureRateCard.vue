      ...
      <v-col cols="12" md="6">
        <v-row class="cycle-metrics">
          <v-col cols="12" class="cycle-metric-time" style="text-align: center; padding: 20px">
            ...
            <span
              class="time-change negative"
              style="font-size: 1.5em; color: red; margin-left: 10px"
              >▲ 28.9 %</span
            >
          </v-col>
          <v-col cols="12">
            <v-row justify="space-between" class="metric-breakdown">
              <v-col cols="6" class="text-center">
                <small
                  >Failed Deployments: <b>{{ failedDeployments }}</b></small
                >
              </v-col>
              <v-col cols="6" class="text-center">
                <small
                  >Total Deployments: <b>{{ totalDeployments }}</b></small
                >
              </v-col>
            </v-row>
          </v-col>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const failedDeployments = computed(() => {
      return (
        props.statuses?.filter(status => status.state === 'failure' || status.state === 'error')
          .length || 0
      )
    })
      ...
      datasets: [
        {
          ...
          data:
            props.statuses?.map(status =>
              status.state === 'failure' || status.state === 'error' ? 1 : 0
            ) || [],
