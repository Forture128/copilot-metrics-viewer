  ...
  <div class="space-y-6">
    ...
    <div class="grid gap-4 grid-cols-12">
      <MetricCard
        ...
        subtitle="Currently assigned seats"
        :value="totalSeats.length"
      ...
      <MetricCard
        ...
        subtitle="No show seats"
        :value="NoshowSeats.length"
        icon="mdi-account-cancel"
        :trend="-1.2"
        description="1.2% decrease from last month"
      />
      <MetricCard
        title="No Activity in Last 7 Days"
        subtitle="No use in the last 7 days"
      ...
      <MetricCard
        ...
        subtitle="For seats with activity"
        :value="averageTimeToFirstActivity"
        icon="mdi-clock-start"
        :trend="-3.4"
        description="3.4% faster than last month"
      />
    </div>
    ...
    <v-container fluid class="charts-container">
      <v-row>
        ...
        <ChartCard
          title="Seats Activity Recency"
          :data="recencyChartData"
          :options="recencyChartOptions"
          :sm="12"
          :md="6"
          :chart-type="'bar'"
        />
        ...
        <ChartCard
          title="Seats Inactivity Recency"
          :data="recencyChartInactiveData"
          :options="recencyChartInactiveOptions"
          :sm="12"
          :md="6"
          :chart-type="'bar'"
        />
      ...
      <v-row>
        ...
        <v-col cols="12" md="4">
          <v-select
            ...
            label="Filter by Activity Status"
          />
        </v-col>
        <v-col v-if="activityStatus === 'Active' || activityStatus === 'Inactive'" cols="12" md="4">
          <v-select
            v-model="recencyFilter"
            :items="recencyFilterOptions"
  ...
  setup(props) {
      ...
      for (let d = new Date(minDate); d <= today; d.setDate(d.getDate() + 1)) {
        ...
        totalSeats.value.forEach(seat => {
          ...
          if (createdDate <= currentDate) {
            ...
            if (
              !lastActivityDate ||
              new Date(currentDate).getTime() - new Date(lastActivityDate).getTime() >
                7 * 24 * 60 * 60 * 1000
            ) {
        ...
        datasets: [
          {
            ...
            data: [
              inactiveCounts['> 30 Days'],
              inactiveCounts['Last 30 Days'],
              inactiveCounts['Last 7 Days'],
              inactiveCounts['Today']
            ],
      ...
      totalSeats.value.forEach(seat => {
        ...
        if (lastActivityAt && !isNaN(createdAt.getTime()) && !isNaN(lastActivityAt.getTime())) {
          const timeDifference =
            (lastActivityAt.getTime() - createdAt.getTime()) / (1000 * 60 * 60 * 24)
          if (timeDifference > 0) {
            timeToFirstActivityData.push(timeDifference)
          }
        }
      })
      const avgTimeToActivity =
        timeToFirstActivityData.reduce((a, b) => a + b, 0) / timeToFirstActivityData.length
    ...
    watchEffect(() => {
      if (props.seats && Array.isArray(props.seats)) {
        ...
        unusedSeats.value = totalSeats.value.filter(
