  ...
  <v-card class="metric-card">
    <v-card-title class="dora-title" style="text-align: left">
      ...
      <v-tooltip bottom>
        ...
        <span
          >Lead time for changes measures the time it takes from committing code to deploying it to
          production.</span
        >
      </v-tooltip>
    </v-card-title>
    <v-row>
      ...
      <v-col cols="12" md="6">
        <v-row class="cycle-metrics">
          <v-col cols="12" class="cycle-metric-time" style="text-align: center; padding: 20px">
            <span class="main-time" style="font-size: 2em; font-weight: bold">{{
              formattedLeadTime
            }}</span>
            <span
          ...
          <v-col cols="12">
            <v-row justify="space-between" class="metric-breakdown">
              <v-col cols="12" class="text-center">
                <small
                  >Average Lead Time: <b>{{ formattedLeadTime }}</b></small
                >
              </v-col>
            </v-row>
          </v-col>
      ...
      <v-col cols="12" md="6">
        <div style="width: 100%; position: relative">
          <LeadTimeLine :data="leadTimeChartData" :options="chartOptions" />
        </div>
      </v-col>
    </v-row>
  </v-card>
  ...
  setup(props) {
    ...
    const formattedLeadTime = computed(() => {
      ...
      const averageLeadTime =
        props.pullRequests.length > 0 ? (totalLeadTime / props.pullRequests.length).toFixed(2) : 0
