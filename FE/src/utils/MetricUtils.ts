...
export const calculateTeamAcceptanceRate = (teamMetrics: Metrics[]): number => {
  const totalSuggestions = teamMetrics.reduce((sum, m) => sum + m.total_suggestions_count, 0)
  const totalAcceptances = teamMetrics.reduce((sum, m) => sum + m.total_acceptances_count, 0)
